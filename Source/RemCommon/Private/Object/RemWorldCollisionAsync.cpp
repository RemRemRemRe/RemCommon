...
REM_DEFINE_PRIVATE_MEMBER_ACCESSOR(GWorldAsyncTraceStateAccessor, &UWorld::AsyncTraceState,
    FWorldAsyncTraceState UWorld::*);
...
struct FBufferIndexPair
{
    ...
    DatumType*
    DatumLookup(TArray<TUniquePtr<TTraceThreadData<DatumType>>>& Array) const
    {
    ...
    template <typename DatumType>
    FORCEINLINE DatumType&
    DatumLookupChecked(TArray<TUniquePtr<TTraceThreadData<DatumType>>>& Array) const
    {
...
std::conditional_t<
    QueryTraceData == EQueryTraceData::Yes, FTraceDatum*, FOverlapDatum*> QueryDataInternal(const UObject& WorldContext,
