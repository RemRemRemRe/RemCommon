...
constexpr bool IsDirectionPerpendicular(const ERemCardinalDirection First,
    const ERemCardinalDirection Second)
{
    using Underlying = std::underlying_type_t<ERemCardinalDirection>;

    const auto ValueFirst  = static_cast<Underlying>(First);
    const auto ValueSecond = static_cast<Underlying>(Second);
