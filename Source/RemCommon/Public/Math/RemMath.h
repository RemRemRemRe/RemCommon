...
template <class T>
[[nodiscard]] constexpr auto CombineNumericRanges(const UE::Math::TVector2<T>& Range1,
    const UE::Math::TVector2<T>& Range2)
{
...
struct TAngle
{
    ...
    explicit constexpr TAngle(const TValue Angle)
        : Angle(Angle)
    {
    }
...
[[nodiscard]] constexpr T RemapRotationForCounterClockwiseRotation(const T& Rotation)
{
    constexpr auto RemapThreshold{
        MakeVectorRegisterConstant(180.0f - CounterClockwiseRotationAngleThreshold,
            180.0f - CounterClockwiseRotationAngleThreshold,
...
template <typename T>
    requires std::is_floating_point_v<T>
[[nodiscard]] constexpr UE::Math::TRotator<T> DampRotatorWithHalfLife3(const UE::Math::TRotator<T>& Current,
    const UE::Math::TRotator<T>& Target,
    ...
    const FVector3f HalfLife3)
{
    ...
    {
        static_assert(always_false<T>::value, "there might be a better way to handle it");
...
[[nodiscard]] constexpr ValueType SpringDamper(StateType& SpringState, const ValueType& Current,
 ...
 */
template <typename T>
[[nodiscard]] constexpr auto CombineRotator(const UE::Math::TRotator<T>& Left,
    const UE::Math::TRotator<T>& Right)
{
