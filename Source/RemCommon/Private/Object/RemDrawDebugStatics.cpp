...
{
    const auto* World{IsValid(Actor)
                          ? Actor->GetWorld()
                          : nullptr
    };
    const auto* Player{IsValid(World)
                           ? World->GetFirstPlayerController()
                           : nullptr
    };
    auto* Hud{IsValid(Player)
                  ? Player->GetHUD()
                  : nullptr
    };
...
void URemDrawDebugStatics::DrawSphereAlternative(const UObject* WorldContext, const FVector& Location,
    ...
    const float Thickness, const uint8 DepthPriority)
{
    ...
    DrawCircle(World, Location, XAxis, YAxis, FColor, Radius, DrawCircleSidesCount, bPersistent, Duration,
        DepthPriority, Thickness);
    DrawCircle(World, Location, XAxis, ZAxis, FColor, Radius, DrawCircleSidesCount, bPersistent, Duration,
        DepthPriority, Thickness);
    DrawCircle(World, Location, YAxis, ZAxis, FColor, Radius, DrawCircleSidesCount, bPersistent, Duration,
        DepthPriority, Thickness);
#endif
}

void URemDrawDebugStatics::DrawSweepSphere(const UObject* WorldContext, const FVector& Start, const FVector& End,
    const float Radius,
    const FLinearColor& Color, const float Duration, const float Thickness, const uint8 DepthPriority)
{
    ...
    DrawDebugDirectionalArrow(World, Start, End, DrawArrowSize, FColor, bPersistent, Duration, DepthPriority,
        Thickness);
#endif
}

void URemDrawDebugStatics::DrawLineTraceSingle(const UObject* WorldContext, const FVector& Start, const FVector& End,
    const bool bHit,
...
void URemDrawDebugStatics::DrawSweepSingleCapsuleAlternative(const UObject* WorldContext, const FVector& Start,
    const FVector& End,
    ...
    const float Duration, const float Thickness, const uint8 DepthPriority)
{
    ...
    DrawQuarterCircle(World, StartBottom, XAxisInverse, ZAxisInverse, Radius, SweepFColor, Duration, Thickness,
        DepthPriority);
    ...
    const float AngleToRotate, const FVector2f& CrossHairCenterScreenSpace, const FLinearColor& LineColor)
{
    ...
    const auto DirectionX = Math::IsNearlyZero(AngleToRotate)
                                ? FVector2f{1.0f, 0.0f}
                                : GetRotatedDirection(FVector2f{1.0f, 0.0f});

    const auto DirectionY = Math::IsNearlyZero(AngleToRotate)
                                ? FVector2f{0.0f, 1.0f}
    ...
    const float DrawTime, const FLinearColor& TraceColor, const FLinearColor& TraceHitColor)
{
        ...
        CollisionParameters.CollisionShape.IsSphere())
    {
        ...
        {
            const FHitResult HitResult{Results.Num() > 0
                                           ? Results[0]
                                           : FHitResult{}
            };
...
void DrawDebugOverlapData(const TNotNull<const UWorld*> World, const FOverlapDatum& Datum,
    const EDrawDebugTrace::Type DrawDebugType,
    const float DrawTime, const FLinearColor& TraceColor, const FLinearColor& TraceHitColor)
{
